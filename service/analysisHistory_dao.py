...
class AnalysisHistoryDAO(BaseService):
    ...
    def get_analysisHistory_info(self, page=1, per_page=10, all=False):
        ...
        stmt = text("select a.id, a.documentCode, a.collectionCode, a.status, DATE_FORMAT(a.created_date, '%d/%m/%y %T') as created_date, d.title, an.name as announcementName "
    ...
    def create_analysisHistory(self, documentId, status, collectionId=""):
        """
        Creates an analysisHistory.
        :param data: analysisHistory's properties as json.
        :return:
        """
        analysisHistory = AnalysisHistory(documentCode=documentId, status=status, collectionCode=collectionId)
